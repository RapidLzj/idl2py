...
def sixty(val):
    ...
    The output has same structure as input
    ** For negative value, h/d, m, and s are all negative **
    :param val: decimal value(s)
    :return: hour_deg, minute, second
    """
    # all these operations will keep the shape of input, even if scalar
    # extract the sign and then use abs value
    sg = np.sign(val)
    absval = np.abs(val)
    # seperate the hour/deg, minute, and second
    hd = np.fix(absval)
    mn = np.fix((absval - hd) * 60.0)
    se = (absval - hd - mn / 60.0) * 3600.0
    # apply the sign
    hd *= sg
    mn *= sg
    se *= sg
